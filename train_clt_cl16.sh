...
#
# With 2 GPUs (DP=1, tp=2):
#   - SAE params: ~40 GB per GPU (sharded across TP dim)
#   - adam8 states: ~10 GB per GPU
#   - ESM2 frozen: ~2.6 GB
#   - Batch + backward: ~10-12 GB (backward also split by TP)
#   - Total: ~62-65 GB per GPU
#
# DP=1 means no data parallelism — single effective worker,
# just the SAE model is split across 2 GPUs.
# =============================================
...
uv run torchrun --nproc_per_node=$NGPUS \
  ...
  --loss_fn fvu \
  --run_name esm2_clt_b8_s1m_k64_cl16_dp1tp2 \
  --cross_layer 16 \
  --transcode True \
  --expansion_factor 10 \
  --dtype bfloat16 \
